...
$(document).ready(function () {
  ...
  $("#select_todas").selectmenu({
    select: function (event, ui) {
      ga("send", "event", "Filtro películas", "click", ui.item.label);
    },
  });

  $("#select_asignaturas").selectmenu({
    select: function (event, ui) {
      ga("send", "event", "Filtro películas", "click", ui.item.label);
    },
  });

  $("#select_etiquetas").selectmenu({
    select: function (event, ui) {
      ga("send", "event", "Filtro películas", "click", ui.item.label);
    },
  });
  ...
  $(document).on("click", "#select_todas-menu li", function () {
    location.href =
      ...
      lang +
      "/peliculas/featured/" +
      get_alias($("#select_todas option:selected").text()) +
      "-" +
      $("#select_todas").val();
  });
  $(document).on("click", "#select_asignaturas-menu li", function () {
    location.href =
      ...
      lang +
      "/peliculas/subject/" +
      get_alias($("#select_asignaturas option:selected").text()) +
      "-" +
      $("#select_asignaturas").val();
  });
  $(document).on("click", "#select_etiquetas-menu li", function () {
    location.href =
      ...
      lang +
      "/peliculas/topic/" +
      get_alias($("#select_etiquetas option:selected").text()) +
      "-" +
      $("#select_etiquetas").val();
  });
  ...
  /*menu fijo home*/
  if ($(".home #main_menu").length) {
    var menu = $(".home #main_menu").offset().top;
  }
...
async function query(endpoint, ids) {
  const idList = ids.join(","); // Convertimos el array de IDs a una cadena separada por comas
  const url = `https://webadmin.cinescuela.org/wp-json/wp/v2/${endpoint}?include=${idList}&per_page=100`;
...
async function complexQuery(
  ...
  const url = `https://webadmin.cinescuela.org/wp-json/wp/v2/${endpoint}?${queryString}`;
  ...
  // Retrieve total pages and total posts from response headers
  const totalPages = response.headers.get("x-wp-totalpages");
  const totalPosts = response.headers.get("x-wp-total");

  return { response: data, total_pages: totalPages, total_posts: totalPosts };
}
